...
function Home() {
  ...
  // Section arrays — seeded with defaults immediately, Firestore overrides arrive async
  const galleryImages             = pageImages['home-gallery'];
  const trainingCarouselImages    = pageImages['home-training-carousel'];
  const maintenanceCarouselImages = pageImages['home-maintenance-carousel'];
  const salesCarouselImages       = pageImages['home-sales-carousel'];
  const flyingBg      = pageImages['home-flying-bg']?.[0]?.url;
  const maintenanceBg = pageImages['home-maintenance-bg']?.[0]?.url;
  const salesBg       = pageImages['home-sales-bg']?.[0]?.url;

  // Training carousel — text stays hardcoded, images come from admin
  const trainingSlides = [
    { title: 'Discovery Flight',         description: 'Flying is the ultimate dream pursued by mankind. Should you want to live it for real, the discovery flight will certainly fulfil your expectations: After a pre-flight briefing, you will enjoy a full hands-on flying experience with one of our instructors.', cta: 'Learn More', link: '/training/trial-lessons' },
    { title: 'Private Pilot Licence',    description: 'Let aside the ground exams that most students self study before taking the tests on site, the obtention of a PPL(H) requires a minimum of 45 hrs of flight training, including 10 hrs of solo. The duration varies depending on commitment.', cta: 'Learn More', link: '/training/ppl' },
    { title: 'Commercial Pilot Licence', description: 'Holding a Commercial Pilot Licence, CPL(H) gives you the status of professional helicopter pilot. To achieve this, 155 hrs of flying time post licence is required, of which 50 hrs must be Pilot In Command (PIC).', cta: 'Learn More', link: '/training' },
    { title: 'Type Rating',              description: 'Having achieved your PPL(H), you may wish to fly different types of helicopter. A type specific ground training course followed by a minimum of 5 Hrs of flight training will suffice to put you to the Type Rating test.', cta: 'Learn More', link: '/training' },
    { title: 'Night Rating',             description: 'Holding a night rating gives you the most flexibility, as sunset will no longer prevent you from flying. To achieve this, 100 hrs of flying post licence is required, of which 60 Hrs must be Pilot In Command.', cta: 'Learn More', link: '/training' },
    { title: 'Self-Fly Hire',            description: 'With an impressive fleet of over 30 helicopters, HQ will certainly be able to facilitate your flying requirements, either on a short term hiring or on a long term leasing basis.', cta: 'Learn More', link: '/services' },
  ].map((slide, i) => ({ ...slide, image: trainingCarouselImages[i]?.url }));

  // Maintenance carousel — text stays hardcoded, images come from admin
