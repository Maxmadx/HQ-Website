 ...
 * Merges posts.json (static) with Firestore blog_posts (admin-created).
 * Firestore posts take priority when the same slug exists in both sources.
 */
...
function Blog() {
  ...
  useEffect(() => {
    ...
    // Fetch published posts from Firestore (admin-created or seeded)
    const q = query(
    ...
    getDocs(q)
      .then((snap) => {
        setFirestorePosts(snap.docs.map((d) => ({ ...d.data(), id: d.id })));
      })
      .catch(() => {
  ...
  const mergedPosts = [
    // Static posts — replaced by Firestore version if available
    ...staticPosts
      .filter((p) => p.published && p.title)
      .map((p) => {
        const fs = firestoreBySlug[p.id];
        if (!fs) return p;
        // Use Firestore metadata fields when available
        return {
          ...p,
          title:       fs.title       || p.title,
          excerpt:     fs.excerpt     || p.excerpt,
          category:    fs.category    || p.category,
          readingTime: fs.readingTime || p.readingTime,
          image:       fs.coverImage  || p.image,
          date:        fs.publishedAt?.toDate
            ? fs.publishedAt.toDate().toISOString().split('T')[0]
            : p.date,
        };
      }),
