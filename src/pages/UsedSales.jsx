...
// Aircraft listings are now loaded live from Firestore (see UsedSales component below)
  ...
  return (
    <Link to={`/sales/pre-owned/${h.id}`} className="usv__card" style={{ textDecoration: 'none', color: 'inherit' }}>
      ...
      <div className="usv__card-body">
        ...
        <div className="usv__card-meta"><span>{h.year}</span><span className="usv__card-reg">{h.registration}</span>{(h.specs?.hours || h.hoursSinceOverhaul) && <span>{h.specs?.hours || h.hoursSinceOverhaul} hrs TT</span>}</div>
      </div>
    </Link>
  );
}
      ...
      <div className="used-final-cta__container">
        ...
        <Reveal delay={0.2}>
          <div className="used-final-cta__actions">
            <a href="tel:+441895833373" className="used-final-cta__phone">
...
function UsedSales() {
  const { docs: listings, loading: listingsLoading } = useCollection('listings');
  const availableHelicopters = listings.filter(l => l.status !== 'sold');
  const soldHelicopters = listings.filter(l => l.status === 'sold');
  ...
  return (
    <div className="usv usv--v1">
      ...
      <section className="usv__listings">
        ...
        ) : availableHelicopters.length === 0 ? (
          <p style={{ padding: '2rem', color: '#6b7280' }}>No aircraft currently available. Check back soon.</p>
        ) : (
