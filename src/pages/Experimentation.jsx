...
import { useState, useEffect, useRef, useCallback, Fragment, startTransition } from 'react';
import { createPortal } from 'react-dom';
import { Link } from 'react-router-dom';
import { motion, useScroll, useTransform, useInView } from 'framer-motion';
import HeroSectionFinalTesting from './HeroSectionFinalTesting-v2';
...
const SelfFlyHireSection = () => {
  ...
  // Direct-DOM refs for the 3 timeline-step elements. tlProgress used to be a
  // continuous-value state that fired setState on every scroll; now the scroll
  // handler toggles the active class on these refs instead.
  const tlStep1Ref = useRef(null);
  const tlStep2Ref = useRef(null);
  const tlStep3Ref = useRef(null);
  ...
  useEffect(() => {
    ...
    const handleScrollRaw = () => {
      ...
      if (pastSticky <= 0) {
        ...
        if (tlStep1Ref.current) tlStep1Ref.current.classList.remove('sfh-map__tl-step--active');
        if (tlStep2Ref.current) tlStep2Ref.current.classList.remove('sfh-map__tl-step--active');
        if (tlStep3Ref.current) tlStep3Ref.current.classList.remove('sfh-map__tl-step--active');
        if (!isHoveringDest.current && lockedDest === null) {
          startTransition(() => setScrollDest(null));
        }
        return;
      }
      ...
      // Direct-DOM writes only — no setState. classList.toggle short-circuits if
      // the class state matches, so steady-state holds are free.
      if (sfhJourneyRef.current) {
        sfhJourneyRef.current.style.setProperty('--tl-progress', progress);
      }
      if (tlStep1Ref.current) tlStep1Ref.current.classList.toggle('sfh-map__tl-step--active', progress > 0.02);
      if (tlStep2Ref.current) tlStep2Ref.current.classList.toggle('sfh-map__tl-step--active', progress > 0.33);
      if (tlStep3Ref.current) tlStep3Ref.current.classList.toggle('sfh-map__tl-step--active', progress > 0.66);

      // Step 1 + 2: Destination/Aircraft cycling. Wrapped in startTransition so
      // the parent re-render is interruptible — scroll never blocks waiting for
      // it to finish.
      startTransition(() => {
        if (!isHoveringDest.current && lockedDest === null) {
    ...
    <section className="sfh-map" id="fleet" ref={sfhSectionRef} data-cms-section="home-sfh-fleet">
      ...
      <div className="sfh-map__inner">
        ...
        <div className="sfh-map__journey" ref={sfhJourneyRef}>
          ...
          <div ref={tlStep2Ref} className="sfh-map__tl-step sfh-map__tl-step-02">
          ...
          <div ref={tlStep3Ref} className="sfh-map__tl-step sfh-map__tl-step-03">
...
function Experimentation() {
  ...
  useEffect(() => {
    ...
    const handleScrollRaw = () => {
      ...
      if (!isMobile) {
        ...
        if (isStuck && !wasStuck) {
          wasStuck = true;
          startTransition(() => setDealerAutoExpand(true));
        } else if (!isStuck && wasStuck) {
          wasStuck = false;
          startTransition(() => setDealerAutoExpand(false));
        }
        ...
        if (dealerRect.bottom < vh - 200) startTransition(() => setDealerAutoExpand(true));
  ...
  useEffect(() => {
    ...
    const handleScrollRaw = () => {
      ...
      if (!isMobile) {
        ...
        if (isStuck && !wasStuck) {
          wasStuck = true;
          startTransition(() => setServiceAutoExpand(true));
        } else if (!isStuck && wasStuck) {
          wasStuck = false;
          startTransition(() => setServiceAutoExpand(false));
        }
        if (textFadeEl) {
          const textRect = textFadeEl.getBoundingClientRect();
          const earlyStart = 200;
        ...
        if (serviceRect.bottom < vh - 200) startTransition(() => setServiceAutoExpand(true));
        if (textFadeEl) textFadeEl.style.opacity = '1';
        if (titleFadeEl) titleFadeEl.style.opacity = '1';
        if (servicesListEl) servicesListEl.style.opacity = '1';
        serviceEl.style.transform = 'none';
  ...
  // Direct-DOM refs: clubhouseTextPhase is written to these every scroll instead
  // of triggering React renders. Was the biggest contributor to the 350-450ms
  // scroll re-render storm.
  const clubTitleRef    = useRef(null);
  const clubTitleAltRef = useRef(null);
  const clubPhase1Ref   = useRef(null);
  const clubPhase2Ref   = useRef(null);
  ...
  // Clubhouse: right side overlay + title crossfade as you scroll (desktop only).
  // Writes directly to refs — no setState, no re-render, no fiber traversal.
  // Was 2 setState calls per scroll event; now 0.
  useEffect(() => {
    let clubRaf = 0;
    let prevPhasePtr = -1; // tracks pointer-events flip at the 0.5 threshold
    const update = () => {
      ...
      const inv = 1 - textPhase;

      const t1 = clubTitleRef.current;
      const t2 = clubTitleAltRef.current;
      const p1 = clubPhase1Ref.current;
      const p2 = clubPhase2Ref.current;
      if (t1) {
        t1.style.opacity = inv;
        t1.style.transform = `translateY(${-textPhase * 10}px)`;
      }
      if (t2) {
        t2.style.opacity = textPhase;
        t2.style.transform = `translateY(${inv * 10}px)`;
      }
      if (p1) p1.style.opacity = inv;
      if (p2) p2.style.opacity = textPhase;

      // pointer-events flips once at the 0.5 boundary; only write on change.
      const ptr = textPhase > 0.5 ? 1 : 0;
      if (ptr !== prevPhasePtr) {
        prevPhasePtr = ptr;
        if (p1) p1.style.pointerEvents = ptr ? 'none' : 'auto';
        if (p2) p2.style.pointerEvents = ptr ? 'auto' : 'none';
      }
    };
    const onScroll = () => { cancelAnimationFrame(clubRaf); clubRaf = requestAnimationFrame(update); };
    window.addEventListener('scroll', onScroll, { passive: true });
    update(); // prime initial state
  ...
  const navItems = [
    ...
    { id: 'community-wall', label: 'Community & Pricing', shortLabel: 'Pricing', icon: '06', subItems: [] },
  ];
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const navSectionIds = ['training', 'expeditions', 'sales', 'maintenance', 'contact', 'community-wall'];
      ...
      for (const sectionId of navSectionIds) {
        ...
        if (element) {
          ...
          // community-wall: once it enters the active zone, keep it active to end of page
          const matches = sectionId === 'community-wall'
      ...
      // Wrapped in startTransition: the nav-item className flip is allowed to
      // wait if the browser is busy with scroll. Was firing on every section
      // boundary crossing during scroll-back and forcing a 350ms render each.
      startTransition(() => setActiveNavSection(currentNavSection));
    };
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const stuckDistance = isStuck
        ...
        : 0;
      // Both flips are className-only on the nav root. Marking as transition so
      // they don't block the rAF for whatever React render they trigger.
      startTransition(() => {
  ...
  return (
    <div className="final-draft" ref={containerRef}>
          ...
          {navItems.map((item) => (
            ...
            >
              ...
              >
                ...
                <span className="fd-nav__item-label">
                  {item.shortLabel ? (
                    <>
                      <span className="fd-nav__item-label-full">{item.label}</span>
                      <span className="fd-nav__item-label-short">{item.shortLabel}</span>
                    </>
                  ) : (
                    item.label
                  )}
        ...
        <div className="clubhouse__inner">
          <div className="clubhouse__sticky">
            ...
            <div className="clubhouse__title-wrap">
              <h3 ref={clubTitleRef} className="clubhouse__title" style={{ opacity: 1, transform: 'translateY(0px)' }}>The Clubhouse</h3>
              <h3 ref={clubTitleAltRef} className="clubhouse__title clubhouse__title--alt" style={{ opacity: 0, transform: 'translateY(10px)' }}>Escape to the Country</h3>
            </div>
            ...
            <div className="clubhouse__phases">
              ...
              <div ref={clubPhase2Ref} className="clubhouse__phase clubhouse__phase--escape" style={{ opacity: 0, pointerEvents: 'none' }}>
      ...
      <style>{`
        ...
        .fd-nav__item-label-short { display: none; }
        @container (max-width: 215px) {
          .fd-nav__item-label-full { display: none; }
          .fd-nav__item-label-short { display: inline; }
        }
