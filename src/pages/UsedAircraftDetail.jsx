...
// Listings are loaded from Firestore — see UsedAircraftDetail component below
...
function UsedAircraftDetail() {
  ...
  useEffect(() => {
    getDoc(doc(db, 'listings', id)).then((snap) => {
      if (snap.exists()) setAircraft({ id: snap.id, ...snap.data() });
  ...
  const statusDisplay = aircraft.status === 'for_sale' ? 'Available'
    : aircraft.status === 'reserved' ? 'Under Offer' : 'Sold';
  const badgeClass = aircraft.status === 'for_sale' ? 'uad-badge--available'
    : aircraft.status === 'reserved' ? 'uad-badge--under-offer' : 'uad-badge--sold';
  const equipment = aircraft.equipment || [];
    ...
    <div className="uad-page">
            ...
            <div className="uad-main-image">
              <div className="uad-carousel">
                {images.map((img, i) => (
                  ...
                  >
                    <Image src={img.url} alt={img.alt || `${aircraft.model} - ${i + 1}`} width={2500} height={1667} sizes="(max-width: 768px) 100vw, 60vw" priority={i === 0} />
                  </div>
                ))}
              ...
              <div className="uad-thumbs">
                {images.map((img, i) => (
                  <div key={i} className={`uad-thumb ${i === currentSlide ? 'uad-thumb--active' : ''}`} onClick={() => goToSlide(i)}>
                    <Image src={img.url} alt={img.alt || ''} width={120} height={80} sizes="120px" />
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="uad-right">
          <header className="uad-header">
            ...
            <div className="uad-meta">
              ...
              {aircraft.specs?.hours && <span>{Number(aircraft.specs.hours).toLocaleString()} Total Hours</span>}
            </div>
            <div className={`uad-badge ${badgeClass}`}>{statusDisplay}</div>
          </header>

          <div className="uad-price-section">
            <div className="uad-price">{aircraft.priceDisplay}</div>
            ...
            <div className="uad-specs-grid">
              ...
              {aircraft.specs?.engine && (
                <div className="uad-spec">
                  ...
                  <div className="uad-spec-value">{aircraft.specs.engine}</div>
                </div>
              )}
              {aircraft.specs?.serial && (
                <div className="uad-spec">
                  <div className="uad-spec-label">Serial</div>
                  <div className="uad-spec-value">{aircraft.specs.serial}</div>
                </div>
              )}
              {aircraft.specs?.hours && (
                <div className="uad-spec">
                  <div className="uad-spec-label">Total Hours</div>
                  <div className="uad-spec-value">{Number(aircraft.specs.hours).toLocaleString()}</div>
                </div>
              )}
              {aircraft.specs?.remainingHours && (
                <div className="uad-spec">
                  <div className="uad-spec-label">Remaining Hours</div>
                  <div className="uad-spec-value">{Number(aircraft.specs.remainingHours).toLocaleString()}</div>
                </div>
              )}
              {aircraft.specs?.remainingLife && (
                <div className="uad-spec">
                  <div className="uad-spec-label">Remaining Life</div>
                  <div className="uad-spec-value">{aircraft.specs.remainingLife}</div>
                </div>
              )}
              {aircraft.specs?.avionics && (
                <div className="uad-spec">
                  <div className="uad-spec-label">Avionics</div>
                  <div className="uad-spec-value">{aircraft.specs.avionics}</div>
                </div>
              )}
            </div>
          </div>
      ...
      {lightboxOpen && (
        <div className="uad-lightbox" onClick={() => setLightboxOpen(false)}>
          ...
          <Image src={images[currentSlide]?.url} alt={images[currentSlide]?.alt || aircraft.model} onClick={(e) => e.stopPropagation()} width={2500} height={1667} sizes="100vw" />
        </div>
      )}
    </div>
  );
